...
public class AStarPlayer : MonoBehaviour
{
    ...
    void Awake()
    {
       ...
       // UIManager.Instance.AddUIEvent(UIHandler);
    }
    ...
    public void PositionTarget(Vector3 position, float duration, Action callback)
    {
      //  CatGameTools.UIGameObjectPositionBack(transform, position, duration, callback);
    }

    private void UIHandler(bool isShow)
    {
        if (isShow)
            animator.speed = 0;
        else
    ...
    {
        //Debug.Log("OnPathComplete = " + p.error);
        if (!p.error)
        {
            currentWayPoint = 0;
            path = p;
    ...
    void FixedUpdate()
    {
        ...
        Vector3 currentWayPointV = new Vector3(path.vectorPath[currentWayPoint].x, path.vectorPath[currentWayPoint].y, path.vectorPath[currentWayPoint].z);

        // Debug.Log(path.vectorPath[currentWayPoint].y);

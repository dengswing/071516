...
public class tk2dTileMapSceneGUI
{
	...
	{	
		...
		if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushLand ||
			tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushTower ||
			tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushMonster ||
			tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.Brush ||
			tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushRandom) {
			workTiles = true;
		}
	...
	void RectangleDragEnd() {
		...
		if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushLand)
		{
			if (WorkingBrush.tiles.Length == 0)
			{
				return;
			}
			var layer = tileMap.Layers[editorData.layer];

			// Add waypoints
			GameObject pathObj = GameObject.Find("TD_"+layer.gameObject.name);
			if (pathObj == null)
			{
				pathObj = new GameObject();
				Debug.Log("pathObj.name=" + pathObj.name);
                pathObj.name = "TD_" + layer.gameObject.name;
				Debug.Log("pathObj.name=" + pathObj.name);
				GameObject terrainObj = GameObject.Find("TDLogic");
				if (terrainObj != null)
				{
					pathObj.transform.parent = terrainObj.transform;
				}
				else
				{
					Debug.LogError("You must add DoraTD First!!!");
					return;
				}
			}

			PathTD path = pathObj.GetComponent<PathTD>();
			if (path == null)
			{
				path = pathObj.AddComponent<PathTD>();
				path.dynamicOffset = 0;
				path.createPathLine = false;
            }

			GameObject spawnMgrObj = GameObject.Find("SpawnManager");
			if (spawnMgrObj == null)
			{
				Debug.LogError("There is no SpawnManager, Please Create DoraTD FIRST!!!");
				return;
			}
			SpawnManager spawnMgr = spawnMgrObj.GetComponent<SpawnManager>();
			if (spawnMgr == null)
			{
				Debug.LogError("There is no SpawnManager, Please Create DoraTD FIRST!!!");
				return;
			}
			if (spawnMgr.defaultPath == null)
			{
				spawnMgr.defaultPath = path;
            }

			// remove null wp
			ClearNullWayPoint(path);

			foreach (var tile in WorkingBrush.tiles)
			{
				Vector3 tilePos = tileMap.GetTileCenterPosition(tile.x, tile.y);
				// waypoint
				RemoveWayPointByTilePos(path, tilePos);
			}

			// Sort all point
			Vector3 lastWayPointPos;
			if (path.wpList.Count == 0)
			{
				lastWayPointPos = tileMap.GetTileCenterPosition(0, 0);
			}
			else
			{
				lastWayPointPos = path.wpList[path.wpList.Count - 1].position;
			}
			SortedList<float, Vector3> listTilePos = new SortedList<float, Vector3>();
			foreach (var tile in WorkingBrush.tiles)
			{
				Vector3 tilePos = tileMap.GetTileCenterPosition(tile.x, tile.y);
				listTilePos.Add(Vector3.Distance(lastWayPointPos, tilePos), tilePos);
			}

			// add to last

			foreach (Vector3 pos in listTilePos.Values)
			{
				AddWayPointByTilePos(path, pos);
			}
		}
		#endregion

		#region Brush Tower
		if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushTower)
		{
			if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushTower)
			{
				GameObject buildManagerObj = GameObject.Find("BuildManager");
				if (buildManagerObj == null)
				{
					Debug.LogError("You must add DoraTD First!!!");
					return;
				}
				BuildManager buildManager = buildManagerObj.GetComponent<BuildManager>();
				...
				GameObject obj = (GameObject)PrefabUtility.InstantiatePrefab(Resources.Load("ScenePrefab/PlatformTD", typeof(GameObject)));
				//obj=GameObject.CreatePrimitive(PrimitiveType.Plane); 
				obj.transform.parent = buildManagerObj.transform;
				obj.name = "BuildPlatform" + buildManager.buildPlatforms.Count;
				...
				for (int y = 0; y < numTilesY; ++y)
				{
					...
					{
						centerPos += tileMap.GetTileCenterPosition(x0 + x, y0 + y);
					}
				}

				obj.transform.localScale = new Vector3(numTilesX, 1f, numTilesY);
				obj.transform.position = centerPos / (numTilesX * numTilesY);

				obj.name = "BuildPlatform" + buildManager.buildPlatforms.Count;
				buildManager.buildPlatforms.Add(obj.GetComponent<PlatformTD>());
			}
		}
		#endregion

		#region Brush Monster
		if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushMonster)
		{
		...
		if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.Erase || tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.Cut) {
			...
			if (scratchpadGUI.workingHere) {
                ...
                if (pathObj != null)
				{
					path = pathObj.GetComponent<PathTD>();
				}
				GameObject buildManagerObj = GameObject.Find("BuildManager");
				if (buildManagerObj != null)
				{
					buildManager = buildManagerObj.GetComponent<BuildManager>();
				}
				...
				for (int y = 0; y < numTilesY; ++y)
				{
					for (int x = 0; x < numTilesX; ++x)
					{
						Vector3 tilePos = tileMap.GetTileCenterPosition(x0 + x, y0 + y);

						RemoveWayPointByTilePos(path, tilePos);

						RemoveBuildPlatformByTilePos(buildManager, tilePos);
                    }
				}
	...
	void RemoveBuildPlatformByTilePos(BuildManager buildMgr, Vector3 tilePos)
	{
		if (buildMgr == null || tilePos == null)
		{
			return;
		}

		GameObject delobj = null;
		for (int i = buildMgr.buildPlatforms.Count - 1; i >= 0; --i)
		{
			float xMin = buildMgr.buildPlatforms[i].transform.position.x - buildMgr.buildPlatforms[i].transform.localScale.x * 0.5f;
			float xMax = buildMgr.buildPlatforms[i].transform.position.x + buildMgr.buildPlatforms[i].transform.localScale.x * 0.5f;
			float zMin = buildMgr.buildPlatforms[i].transform.position.z - buildMgr.buildPlatforms[i].transform.localScale.z * 0.5f;
			float zMax = buildMgr.buildPlatforms[i].transform.position.z + buildMgr.buildPlatforms[i].transform.localScale.z * 0.5f;

			if (tilePos.x > xMin && tilePos.x < xMax && tilePos.z > zMin && tilePos.z < zMax)
			{
				delobj = buildMgr.buildPlatforms[i].gameObject;
				buildMgr.buildPlatforms.RemoveAt(i);
				break;
			}
		}
		if (delobj)
			Object.DestroyImmediate(delobj);

		for (int i = 0; i < buildMgr.buildPlatforms.Count; ++i)
		{
			buildMgr.buildPlatforms[i].gameObject.name = "BuildPlatform" + i;
		}
	}


	void ClearNullWayPoint(PathTD path)
	{
		if (path == null)
		{
			return;
		...
		// remove null wp
		for (int i = path.wpList.Count - 1; i >= 0; --i)
		{
			if (path.wpList[i] == null)
			{
				path.wpList.RemoveAt(i);
			}
		}
    }
	void RemoveWayPointByTilePos(PathTD path, Vector3 tilePos)
	{
		if (path == null || tilePos == null)
		{
			return;
		}
		...
		for (int i = path.wpList.Count - 1; i >= 0; --i)
		{
			if (Vector3.Distance(path.wpList[i].position, tilePos) < 1e-6f)
			{
				delobj = path.wpList[i].gameObject;
				path.wpList.RemoveAt(i);
				break;
			}
		}
		...
		for (int i=0; i< path.wpList.Count; ++i)
		{
			path.wpList[i].gameObject.name = path.name + "_WP_" + i;
		}
    }

	void AddWayPointByTilePos(PathTD path, Vector3 tilePos)
	{
		if (path == null || tilePos == null)
		{
			return;
		...
		obj.name = path.name+"_WP_" + path.wpList.Count;
		obj.transform.position = tilePos;
		obj.transform.parent = path.gameObject.transform;

		path.wpList.Add(obj.transform);
	}
	...
	public void OnSceneGUI()
	{
		...
		if (updateCursorColor) {
			switch (tk2dTileMapToolbar.mainMode) {
				case tk2dTileMapToolbar.MainMode.BrushLand:
					tileSelectionFillColor = tk2dPreferences.inst.tileMapToolColor_brush_land;
					break;
				case tk2dTileMapToolbar.MainMode.BrushTower:
					tileSelectionFillColor = tk2dPreferences.inst.tileMapToolColor_brush_tower;
					break;
				case tk2dTileMapToolbar.MainMode.BrushMonster:
					tileSelectionFillColor = tk2dPreferences.inst.tileMapToolColor_brush_monster;
					break;
	...
	public void UpdateWorkingBrush()
	{
		...
		if (tk2dTileMapToolbar.mainMode == tk2dTileMapToolbar.MainMode.BrushLand ||
			...
			if (rectX1 == rectX2 && rectY1 == rectY2) {
				...
				Debug.Log("rectX1 != rectX2 || rectY1 != rectY2 count=" + gridWidth * gridHeight);

...
public class tk2dTileMap : MonoBehaviour, tk2dRuntime.ISpriteCollectionForceBuild
{
	...
	/// <summary>
	/// Returns the tile center position in world space
	/// </summary>
	public Vector3 GetTileCenterPosition(int x, int y)
	{
		switch (data.tileType)
		{
			...
			default:
				{
					Vector3 localPosition = new Vector3(
					x * data.tileSize.x + data.tileOrigin.x + data.tileSize.x * 0.5f,
					y * data.tileSize.y + data.tileOrigin.y + data.tileSize.y * 0.5f,
					0);
					return transform.localToWorldMatrix.MultiplyPoint(localPosition);
				}
			case tk2dTileMapData.TileType.Isometric:
	...
	public void SetTilePrefab(int x, int y, int layer, GameObject instance)
	{
		TilemapPrefabInstance item = new TilemapPrefabInstance();
		item.x = x;
		item.y = y;

	...
	public class UIBuildButton : MonoBehaviour {
		...
		public void OnTowerButton(GameObject butObj){
			...
			if (!UI.UseDragNDrop())
			{
				...
				{
					exception = "Select a Build Point First";
				}
				else if (!GameControl.IsGameStarted())
				{
					exception = "Cant Build Tower before spawn start";
				}
				else
				{
					UnitTower newTower = BuildManager.BuildTower(towerList[ID]);
					if (newTower == null)
					{
		...
		void Update () {
			if(!UI.UseDragNDrop()) {
				...
				// TODO 屏幕适配相关，在屏幕适配下面Camera.main.WorldToScreenPoint 计算错误，需要修正，这里暂时临时处理
				// http://stackoverflow.com/questions/29884883/position-auto-scaled-ui-element-with-camera-worldtoscreenpoint
				float refWidth = 1024; // reference resolution - width - set in Canvas Scaler 
				float refHeight = 768; // reference resolution - height - set in Canvas Scaler

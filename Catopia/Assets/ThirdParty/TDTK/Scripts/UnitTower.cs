...
namespace TDTK {
	...
	public class UnitTower : Unit {
        ...
        public delegate void PlayIdleAnimation();
        public PlayIdleAnimation playIdleAnimation;
		...
		public AudioClip audioSold;
		public AudioClip audioUpgraded;
		public AudioClip audioConstructionStart;
		public AudioClip audioConstructionComplete;

		public GameObject effectSold;
		public GameObject effectUpgraded;
		public GameObject effectConstructionStart;
		public GameObject effectConstructionComplete;
		...
		public void InitTower(int ID){
			...
			if (weapon != null)
			{
				...
				Transform weaponTarget = FindChild(transform, "WeaponBone");
				if (weaponTarget == null)
				{
					Debug.Log("The tower(" + gameObject.name + ") has weapon but not has weapon target node 'weaponbone'!!!");
					return;
				}
				GameObject weaponGo = GameObject.Instantiate(weapon);
				if (weaponGo != null)
				{
					weaponGo.transform.parent = weaponTarget;
					weaponGo.transform.position = weaponTarget.position;
					weaponGo.transform.rotation = weaponTarget.rotation;

					if (stats[currentActiveStat].shootObjectT != null || stats[currentActiveStat].shootObject != null)
					{
						Transform shootTarget = FindChild(weaponGo.transform, "GrenadePoint");
						if (shootTarget == null)
						{
							Debug.Log("The tower("+gameObject.name+")'s weapon("+ weaponGo.name+ ") hasn't shootPoint 'GrenadePoint'!!!");
							return;
						}
						shootPoints.Clear();
						shootPoints.Add(shootTarget);
					}
				}
			}
		}
		...
		public IEnumerator DragNDropRoutine(){
			...
			yield return null;
				...
				if(status==_TileStatus.Available){
					string exception = "";
					UnitTower newTower = BuildManager.BuildTower(srcTower);
					if (newTower == null)
					{
		...
		IEnumerator AOETowerRoutine(){
			...
			while(true){
				...
				List<Unit> tgtList = ScanForTargetByPhysics(thisT.position, GetRange(), maskTarget);
				foreach (Unit unit in tgtList)
				{
		...
		IEnumerator MineRoutine(){
			...
			while(true){
				...
				{
					List<Unit> tgtList = ScanForTargetByPhysics(thisT.position, GetAOERadius(), maskTarget);
					foreach (Unit unit in tgtList)
					{
						AttackInstance attInstance = new AttackInstance();
						attInstance.srcUnit = this;
		...
		Transform FindChild(Transform parent, string name)
		{
			...
			{
				Transform subTrans = parent.GetChild(i);
				if (subTrans.name == name)
				{
					return subTrans;
				}
				else
				{
					Transform subsubTrans = FindChild(subTrans, name);
					if (subsubTrans != null)
					{
						return subsubTrans;
                    }

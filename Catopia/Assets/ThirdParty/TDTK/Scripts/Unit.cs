	...
	public class Unit : MonoBehaviour {
		...
		void ScanForTarget(){
			if(!dead && !IsInConstruction() && !stunned){
				...
				if(directionalTargeting && IsCreep()) dirScanRot=thisT.rotation;
		...
		public List<Unit> ScanForTargetByPhysics(Vector3 position, float range, LayerMask targetMask)
		{
			List<Unit> ret = new List<Unit>();
			Collider[] cols = Physics.OverlapSphere(position, range, targetMask);
			for (int i = 0; i < cols.Length; i++)
			{
				...
				// 策划要求：攻击对象由碰撞盒相交改成中心点距离
				if (Vector3.Distance(cols[i].transform.position, position) <= GetRange())
				{
					Unit unit = cols[i].gameObject.GetComponent<Unit>();
					if (!unit.dead)
						ret.Add(unit);

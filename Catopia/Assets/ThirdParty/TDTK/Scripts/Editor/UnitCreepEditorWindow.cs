...
namespace TDTK {
	...
	public class UnitCreepEditorWindow : UnitEditorWindow {
		...
		private static void UpdateObjectHierarchyList(){
			...
			if(creepList.Count==0 || selectID>=creepList.Count) selectID = 0;
		...
		void OnGUI () {
			...
			cont = new GUIContent("Creep ID:", "The Creep ID");
			EditorGUI.LabelField(new Rect(startX, startY, width, height), cont);
			string newIDStr = EditorGUI.TextField(new Rect(startX + 90, startY, width - 5, height), creepList[selectID].prefabID.ToString());
			int newID = int.Parse(newIDStr);
			EditorDBManager.AddCreepNewID(creepList[selectID], newID);

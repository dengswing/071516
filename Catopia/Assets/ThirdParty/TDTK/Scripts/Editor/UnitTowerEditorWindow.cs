...
namespace TDTK {
	...
	public class UnitTowerEditorWindow : UnitEditorWindow {
		...
		private static void UpdateObjectHierarchyList(){
			...
			if(towerList.Count==0 || selectID>=towerList.Count) selectID = 0;
		...
		void OnGUI () {
			...
			cont = new GUIContent("Tower ID:", "The Tower ID");
			EditorGUI.LabelField(new Rect(startX, startY, width, height), cont);
			string newIDStr = EditorGUI.TextField(new Rect(startX + 90, startY, width - 5, height), towerList[selectID].prefabID.ToString());
			int newID = int.Parse(newIDStr);
			EditorDBManager.AddTowerNewID(towerList[selectID], newID);
			startY += spaceY + 10;
